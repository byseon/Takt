---
name: takt-next
description: Advance to the next Takt milestone after the current one completes. Triggers on "takt next".
---
...
1. **Current milestone is complete.** Verify all tickets in the current milestone have `approved` status by reading ticket files in `.takt/tickets/milestones/<current-milestone>/`. If any tickets are not yet approved:
   > "Milestone is not yet complete. X tickets still pending approval. Run `/takt-review` to review completed tickets."
2. **State file exists.** Verify `.takt/state/takt-state.json` exists and shows `phase >= 3`.
...
1. Move all ticket files from `.takt/tickets/milestones/<milestone>/` to `.takt/tickets/archive/<milestone>/`.
2. **Bundle ticket artifacts** into the archive directory alongside the tickets:
   - Move `.takt/comms/<ticket-id>-output.md` → `.takt/tickets/archive/<milestone>/<ticket-id>-output.md` for each ticket
   - Move `.takt/reviews/<ticket-id>-review.json` → `.takt/tickets/archive/<milestone>/<ticket-id>-review.json` for each ticket
   - Move `.takt/comms/<ticket-id>-review-output.md` → `.takt/tickets/archive/<milestone>/` (if peer review was used in subagent mode)
...
4. Update `takt-state.json` milestones array entry to reflect completion.
5. Generate a milestone summary in `.takt/logs/M001-summary.md`:
   - What was delivered
   - Metrics (tickets completed, time elapsed, agents involved)
   - Issues encountered and how they were resolved
   - Learnings for future milestones
6. **Update `.takt/HANDOFF.md`** — This is a critical step. Perform a FULL handoff update:
   a. Read the existing `.takt/HANDOFF.md` to preserve Milestone History.
   b. Rewrite the entire file using the current state. Update ALL sections:
      ...
      - **Milestone History** — append a new entry:
        ...
        **Issues:** <bulleted list of issues encountered, or "(none)">
        ```
   c. If this is a manual invocation (user ran `/takt-handoff`), you may also invoke that skill instead.
...
2. **Merge** the agent's branch into main: `git merge takt/<agent-id> --no-ff -m "M001: merge takt-<agent-id> changes"`
...
4. **Remove worktrees** after successful merge: `git worktree remove .worktrees/takt-<agent-id>`
5. **Delete branches**: `git branch -d takt/<agent-id>`

Fresh worktrees are created for the next milestone when `/takt-execute` runs again (Step 3.0).
...
**Protected agents:** The **takt-orchestrator** and **takt-reviewer** are permanent team members and must NEVER be removed during roster review. Every project needs coordination and code review regardless of milestone.
...
2. **Clean up all worktrees** — Remove `.worktrees/` directory and all `takt/*` branches.
3. Update state to `{ "phase": 5, "status": "project-complete" }`.
4. Generate a final project report in `.takt/logs/project-report.md`:
   - Overall metrics
   - Architecture decisions and rationale
   - Known issues and technical debt
   - Recommended next steps
5. Announce to the user:
   > "Takt project complete. All milestones delivered. See `.takt/logs/project-report.md` for the full report."
